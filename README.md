...
If `pyttsx3` fails to initialize, the server falls back to the OS speech command such as `say`, `powershell`, or `espeak` when available.
On Linux this requires the `espeak` command to be installed as a system package.
